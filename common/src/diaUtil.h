...
struct DiaAllowEverythingSettingsCallBack : public IDiaLoadCallback2 {
    const bool withSymbolServer;

    explicit DiaAllowEverythingSettingsCallBack(const bool withSymbolServer) : withSymbolServer(withSymbolServer) {}
    ...
    HRESULT STDMETHODCALLTYPE RestrictSymbolServerAccess() override {
        return withSymbolServer ? S_OK : S_FALSE;
    }
